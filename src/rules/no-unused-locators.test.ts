...
runRuleTester('no-unused-locators', rule, {
  invalid: [
    ...
    },
    {
      ...
      name: 'awaited locator called without usage. Playwright does not fail when a locator results in no elements found.',
    },
  ],
  valid: [
    {
      code: `const btn = page.getByRole('button', { name: 'Sign in' })
        btn.click() // should have "await"
      `,
      name: 'If a variable is assigned a locator and then used but the promise is not properly handled, do not report it as unused (rule is not set up to handle)',
    },
    {
